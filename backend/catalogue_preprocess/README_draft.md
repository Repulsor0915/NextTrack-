...
| File | Responsibility |
|---|---|
| `catalogue_preprocess/schema.py` | Declares source-column mappings and the hard-validation policy. |
| `catalogue_preprocess/validation.py` | Validates and converts one raw CSV row. |
| `catalogue_preprocess/pipeline.py` | Builds the full catalogue, deterministic samples, reports, manifests, and checksums. |
...
Generating processed data and importing it are separate actions. The importer
requires `catalogue.json` and its sibling `manifest.json`, verifies SHA-256 and
byte size, and records one active catalogue version. On a new, empty database:

```powershell
.\.venv\Scripts\python.exe backend\manage.py migrate
.\.venv\Scripts\python.exe backend\manage.py import_catalogue data\processed\spotify-tracks-kaggle-full\catalogue.json --dry-run
...
On an existing database, default import only accepts the identical verified
snapshot. `--prune` is a complete snapshot sync: it updates shared IDs, adds
new IDs, and deletes absent IDs. `--replace` deletes all old catalogue rows and
rebuilds from the snapshot. Both modes require `--dry-run` inspection and
explicit `--confirm-version`, `--confirm-sha256`, and the `--confirm-plan`
token printed by the current preview when rows already exist. Pruning more
than 10% also needs `--allow-large-prune`. Never use the
20/500 test catalogue to replace the full development database. See the
[Stage 4 database guide](../../docs/stage-4-catalogue-database.md) for safe
staging commands, legacy adoption, and PostgreSQL deployment decisions.
...
`catalogue_preprocess.pipeline.prepare_catalogue()`.
...
`catalogue_preprocess.pipeline.sample_catalogue()`. Its central selection is
equivalent to:
...
Running `manage.py import_catalogue` follows a different path. It verifies the
processed catalogue against its manifest, validates the eight-feature records,
builds a read-only import plan, then bulk-writes `Track` and `TrackFeatures`
in one atomic transaction. `CatalogueState` is committed in that same
transaction. It never calls the raw-CSV validation or sampling functions.
