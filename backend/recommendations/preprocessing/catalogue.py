...
from .normalization import FEATURE_RANGES
...
def prepare_catalogue(
    ...
    *,
    ...
    if limit is not None and limit > len(valid_catalogue):
        raise CataloguePreparationError(
            f"limit {limit} exceeds {len(valid_catalogue)} valid unique tracks"
        )

    if limit is None:
        selected = sorted(valid_catalogue, key=lambda item: item["id"])
        selection_mode = "all_valid_unique"
        selection_method = "all valid unique tracks sorted by track_id"
        selection_seed = None
    else:
        selected = sorted(
            valid_catalogue,
        ...
        selection_mode = "deterministic_hash_sample"
        selection_method = (
    ...
    manifest = {
        ...
        "transform": {
            ...
            "source_text_max_lengths": SOURCE_MAX_LENGTHS,
            ...
            "selection_seed": selection_seed,
            "normalization": {
                "stored_values": "raw source feature values",
                "method": "min-max scaling followed by clamping",
...
def _read_and_validate(source_path):
    ...
    with source_path.open(encoding="utf-8-sig", newline="") as source_file:
        ...
        for source_row_number, row in enumerate(reader, start=2):
            ...
            for column_name, maximum_length in SOURCE_MAX_LENGTHS.items():
